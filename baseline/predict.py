...
#     frg_mask_depth = np.logical_and((abs(depth_in-depth_bg) > 0.02), (depth_bg != 0))
...
#     input_points = np.array([cam_x[depth_valid], cam_y[depth_valid], cam_z[depth_valid]])
...
## scale the images to the proper value
rgb_in = np.asarray(rgb_in, dtype=np.float32) / 255
rgb_bg = np.asarray(rgb_bg, dtype=np.float32) / 255
depth_in = np.asarray(depth_in, dtype=np.float64) / 10000
depth_bg = np.asarray(depth_bg, dtype=np.float64) / 10000

## get foreground mask
frg_mask_color = ~(np.sum(abs(rgb_in-rgb_bg) < 0.3, axis=2) == 3)
frg_mask_depth = np.logical_and((abs(depth_in-depth_bg) > 0.02), (depth_bg != 0))
foreground_mask = np.logical_or(frg_mask_color, frg_mask_depth)

## project depth to camera space
pix_x, pix_y = np.meshgrid(np.arange(640), np.arange(480))
cam_x = (pix_x - cam_intrinsic[0][2]) * depth_in/cam_intrinsic[0][0]
cam_y = (pix_y - cam_intrinsic[1][2]) * depth_in/cam_intrinsic[1][1]
cam_z = depth_in
...
surface_normals_map = np.zeros(rgb_in.shape)
n = 0
...
mean_std_norms = np.mean(generic_filter(surface_normals_map, np.std, size=(25,25)), axis=2)
affordance_map = 1 - mean_std_norms/np.max(mean_std_norms[:])
affordance_map[~depth_valid] = 0

np.savetxt('test.txt', surface_normals_map, fmt='%d')
np.savetxt('test1.txt', affordance_map, fmt='%d')
