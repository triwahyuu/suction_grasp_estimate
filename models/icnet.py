    ...
    def __init__(self, in_channels, mid_channels, out_channels, 
        ...
        if dilation > 1:
            self.cbr2 = conv2DBatchNormRelu(
                mid_channels, mid_channels, 3, stride=stride, 
        ...
        self.cb3 = conv2DBatchNorm(
            mid_channels, out_channels, 1, stride=1, 
            padding=0, bias=bias, is_batchnorm=is_batchnorm
        )
...
class bottleNeckIdentifyPSP(nn.Module):
    def __init__(self, in_channels, mid_channels, stride, dilation=1, is_batchnorm=True):
        ...
        self.cb3 = conv2DBatchNorm(
            mid_channels, in_channels, 1, stride=1, 
            padding=0, bias=bias, is_batchnorm=is_batchnorm
        )
        ...
        if include_range in ["all", "conv"]:
            layers.append(
                bottleNeckPSP(
                    ...
                    stride, dilation, is_batchnorm=is_batchnorm,
                )
            )
...
class ICNet(nn.Module):
    ...
    def __init__(self, n_classes=19, block_config=[3, 4, 6, 3], is_batchnorm=True):
    ...
    def forward(self, x, x_sub2, x_sub4):
        ## MID
        ...
        if self.training:
            ...
            return F.interpolate(
                sub124_cls, size=get_interp_size(sub124_cls, z_factor=4),
