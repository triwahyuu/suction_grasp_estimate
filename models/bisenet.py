...
def _build_contextpath_resnet(model_name):
    ...
    if model_name not in avail_resnet:
        raise ValueError('BiseNet backend should be one of: ' +
                         ', '.join(avail_resnet))
...
class AttentionRefinementModule(nn.Module):
    ...
    def forward(self, input):
        ...
        assert self.in_channels == x.size(1), \
            'in_channels ({}) and out_channels ({}) should all be {}'.format(
                self.in_channels, self.out_channels, x.size(1))
        x = self.conv(x)
...
class BiSeNet(nn.Module):
    def __init__(self, num_classes, att_size, bise_size, ffm_size):
        ...
        self.attention_refinement_module2 = AttentionRefinementModule(
            bise_size*2, bise_size*2)
        # supervision block
        self.supervision1 = nn.Conv2d(
            in_channels=att_size, out_channels=num_classes, kernel_size=1)
        self.supervision2 = nn.Conv2d(
            in_channels=bise_size*2, out_channels=num_classes, kernel_size=1)
    ...
    def forward(self, sx, cx1, cx2, tail):
        ...
        cx1 = torch.nn.functional.interpolate(
            cx1, scale_factor=2, mode='bilinear')
        cx2 = torch.nn.functional.interpolate(
            cx2, scale_factor=4, mode='bilinear')
        cx = torch.cat((cx1, cx2), dim=1)

        if sx.size(2) != cx.size(2) and sx.size(3) != cx.size(3):
            cx = torch.nn.functional.interpolate(
                cx, size=(sx.size(2), sx.size(3)), mode='bilinear')

        if self.training == True:
            cx1_sup = torch.nn.functional.interpolate(
                self.supervision1(cx1), scale_factor=8, mode='bilinear')
            cx2_sup = torch.nn.functional.interpolate(
                self.supervision2(cx2), scale_factor=8, mode='bilinear')
